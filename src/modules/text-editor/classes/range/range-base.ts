...
export class RHBase implements RHBaseInterface {
  ...
  private setSelection(): RangeValuesInterface {
    ...
    if (this.range.commonAncestorContainer.hasChildNodes()) {
      rv.ancestorContainsSpan = this.hasSpanNode(this.range.commonAncestorContainer)
    }
  ...
  private hasSpanNode(nodeToCheck: Node): boolean {
    for (const node of nodeToCheck.childNodes) {
      if (this.getNodeType(node) === 'span') return true;
    };
    return false;
  }
...
export class Indents extends RHBase {
  ...
  private applyIndent(direction: 'increase' | 'decrease') {
    if(!this.range) throw new Error('applyIndent: Range not set');
