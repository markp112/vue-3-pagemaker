...
export class CurrentStyling {
  ...
  private createStyle(styleName: StyleTags | CssStyleTypes , value: string): Style {
    const style: Style = { 
      style: styleName,
      value: value
    };
  ...
  private addNodeStyle(styleName: StyleTags | CssStyleTypes, value: string, owner: StyleOwner, type: StyleType) {
  ...
  public setStylesFromNode(node: Node, owner: StyleOwner) {
    ...
    for (let index = 0; index < element.style.length; index++) {
      const styleName = element.style.item(index) as StyleTags | CssStyleTypes;
  ...
  public setClassesFromNode(node: Node, owner: StyleOwner) {
    if (node.nodeName !== 'SPAN') return;
    const className = (node as HTMLElement).className as StyleTags | CssStyleTypes;
